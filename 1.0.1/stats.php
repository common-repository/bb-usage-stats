<?php if (! defined('BB_USAGE_STATS')) return; ?>

<?php

$mapper = bbUsageStatsMapper::Instance();
$stats  = bbUsageStats::Instance();

global $wpdb;
$trackTable = $mapper->tblTrack;
...
$lastDay = $stats->getMonthDays($startDate['mon'], $startDate['year']);
if ($lastDay < $startDate['mday'])
	$startDate['mday'] = $lastDay;

$start_date_fmt = sprintf('%04d-%02d-%02d', $startDate['year'], $startDate['mon'], $startDate['mday']);
$end_date_fmt = sprintf('%04d-%02d-%02d', $endDate['year'], $endDate['mon'], $endDate['mday']);

//-- Data --
$sql = "SELECT SUM(track.counter) AS counter,
               track.object_type AS object_type,
               track.track_date AS track_date
          FROM $trackTable track
         WHERE track.object_id = 0
               AND track.track_date >= '$start_date_fmt'
               AND track.track_date <= '$end_date_fmt'
         GROUP BY track.track_date,
                  track.object_type";

$data = array();
foreach ($wpdb->get_results($sql) as $row) {
    $parts = explode('-', $row->track_date, 2);
    $day = $parts[1];
    $data[$row->object_type][$day] = $row->counter;
}

$allTypeSettings = array(
        BB_USAGE_STATS_HOME => array('Home', '#0DCF8E'),
        BB_USAGE_STATS_PAGE => array('Pages', '#FF9E01'),
        BB_USAGE_STATS_CATEGORY => array('Categories', '#0D8ECF'),
        BB_USAGE_STATS_POST => array('Posts', '#FCD202'),
    );

$types = array();
foreach($allTypeSettings as $type => $typeSettings) {
    list($title, $color) = $typeSettings;
    $types[$type] = 'title="'.esc_attr($title).'" fill_alpha="60" line_width="2" bullet="round" color="'.$color.'"';
}

$chartTitle = "Daily Usage Statistics $start_date_fmt/$end_date_fmt";

$chartSettings = <<< EOSETTINGS
<settings> 
...
$iDay = $startDate['mday'];
$iMonth = $startDate['mon'];
$endDay = $endDate['mday'];
$endMonth = $endDate['mon'];

while ($iMonth < $endMonth || $iDay <= $endDay) {
	$key = sprintf('%02d-%02d', $iMonth, $iDay);
    $chartData .= "<value xid=\"$key\">$key</value>";
	//
	$iDay++;
	if ($iDay > $lastDay) {
		$iDay = 1;
...
$chartData .= '</series><graphs>';

foreach ($types as $objType => $objTypeAttr) {
    $chartData .= "<graph $objTypeAttr>";
	...
	while ($iMonth < $endMonth || $iDay <= $endDay) {
		$key = sprintf('%02d-%02d', $iMonth, $iDay);
        $val = isset($data[$objType][$key]) ? $data[$objType][$key] : 0;
        $chartData .= "<value xid=\"$key\">$val</value>";
		//
		$iDay++;
		if ($iDay > $lastDay) {
			$iDay = 1;
...
$chartSettings = addcslashes(str_replace(array("\n", "\r"), '', $chartSettings), '"');
$chartData = addcslashes(str_replace(array("\n", "\r"), '', $chartData), '"');

?>
    <script type="text/javascript" src="<?php echo BB_USAGE_STATS_URL.'/amline/swfobject.js'; ?>"></script>
	...
	// <![CDATA[		
		var so = new SWFObject("<?php echo BB_USAGE_STATS_URL.'/amline/amline.swf'; ?>", "amline", "520", "400", "8", "#FFFFFF");
		so.addVariable("path", "<?php echo BB_USAGE_STATS_URL.'/amline/'; ?>");
		so.addVariable("chart_data", encodeURIComponent("<?php echo $chartData; ?>"));
		so.addVariable("chart_settings", encodeURIComponent("<?php echo $chartSettings; ?>"));
...
foreach ($allTypeSettings as $type => $typeParams) {
    if ($type == BB_USAGE_STATS_HOME) continue;
    list($typeTitle, $typeColor) = $typeParams;
    if (empty($typeColor))
        $typeColor = '#0D8ECF';
    $sql = "SELECT SUM(track.counter) AS counter,
                   track.object_id AS object_id
              FROM $trackTable track
             WHERE track.track_date >= '$start_date_fmt'
                   AND track.track_date <= '$end_date_fmt'
                   AND track.object_type = '$type'
				   AND object_id <> 0
             GROUP BY track.object_id
             ORDER BY counter DESC
             LIMIT 0, $topNumber";
    $resultRows = $wpdb->get_results($sql);
    $allCounter = 0;
    foreach ($resultRows as $row) {
        $allCounter += $row->counter;
    }
    //-- Output --
    print("<tr><th colspan=\"3\" scope=\"col\" style=\"text-align:left; \"><br /><strong>Top $topNumber $typeTitle</strong></th></tr>");
    foreach ($resultRows as $row) {
        $link = $stats->getItemLink($type, $row->object_id);
        $countPercent = round(100*$row->counter/$allCounter);
        $w = round(250*$countPercent/100);
        print "<tr><td>{$row->counter}</td><td><div style=\"background-color: $typeColor; float:left; width:{$w}px\">&nbsp;</div>$countPercent%</td><td>$link</td></tr>";
    }
}
//-- Data --
